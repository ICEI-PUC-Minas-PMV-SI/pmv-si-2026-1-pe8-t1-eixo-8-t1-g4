...
if (form) {
  form.addEventListener('submit', async function (e) {
    ...
    const palletMadeira = Number(document.getElementById('palletMadeira').value);
    const flexpen     = Number(document.getElementById('flexpen').value);
    const palletTPU   = Number(document.getElementById('palletTPU').value);

    if (cartucho === 0 && bula === 0 && rotulo === 0 && caixa === 0 &&
        palletMadeira === 0 && flexpen === 0 && palletTPU === 0) {
    ...
    try {
      const response = await fetch('http://localhost:3000/solicitacoes', {
        ...
        body: JSON.stringify({
          ...
          linha,
          materiais: { cartucho, bula, rotulo, caixa, palletMadeira, flexpen, palletTPU },
          observacoes
        })
      });
      ...
      form.reset();
      ['cartucho', 'bula', 'rotulo', 'caixa', 'palletMadeira', 'flexpen', 'palletTPU'].forEach(id => {
